...
export async function deleteTravelReport(id: string) {
  ...
  try {
    ...
    if (reportData.estado === 'Aprobado') {
      return { 
        success: false, 
        message: "No se puede eliminar un informe aprobado. Contacta con el administrador." 
      };
    }
    ...
    return { 
      success: true, 
      message: `Informe ${reportData.codigo_informe || id} eliminado correctamente.` 
    };
...
export async function deleteTravelReports(ids: string[]) {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (reportData.estado === 'Aprobado') {
        skippedCount++;
        errors.push(`${reportData.codigo_informe || id} está aprobado`);
        continue;
      }
    ...
    if (skippedCount > 0) {
      message += ` ${skippedCount} informe(s) omitido(s) (aprobados o no encontrados).`;
    }
