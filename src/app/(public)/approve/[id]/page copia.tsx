...
async function getOrderDetails(id: string): Promise<PurchaseOrder | null> {
  try {
    ...
    // Asegurarnos de que los timestamps se convierten para el cliente
    return convertTimestampsToISO({ id: orderSnap.id, ...orderData });

  } catch (error) {
    console.error("Error fetching order details with Admin SDK:", error);
    return null;
  }
}
